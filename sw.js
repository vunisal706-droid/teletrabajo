const CACHE_NAME = 'teletrabajo-capitulaciones-v16';
...
self.addEventListener('install', event => {
  console.log('Service Worker v16: Installing...');
  console.log('Base URL:', baseUrl);
  console.log('URLs to cache:', urlsToCache);
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then(cache => {
        console.log('Service Worker v16: Caching files');
      ...
      .then(() => {
        console.log('Service Worker v16: Skip waiting');
      ...
      return Promise.all(
        cacheNames.map(cacheName => {
          if (cacheName !== CACHE_NAME) {
            console.log('Service Worker v16: Deleting old cache:', cacheName);
            return caches.delete(cacheName);
          }
        })
      );
    }).then(() => {
      console.log('Service Worker v16: Claiming clients');
