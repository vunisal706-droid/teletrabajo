const CACHE_NAME = 'teletrabajo-capitulaciones-v40';
...
self.addEventListener('install', event => {
  ...
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then(cache => {
        console.log('Service Worker v40: Caching files');
      ...
      .then(() => {
        console.log('Service Worker v40: Skip waiting');
      ...
      return Promise.all(
        cacheNames.map(cacheName => {
          if (cacheName !== CACHE_NAME) {
            console.log('Service Worker v40: Deleting old cache:', cacheName);
            return caches.delete(cacheName);
          }
        })
      );
    }).then(() => {
      console.log('Service Worker v40: Claiming clients');
