const CACHE_NAME = 'teletrabajo-capitulaciones-v25';
...
self.addEventListener('install', event => {
  console.log('Service Worker v25: Installing...');
  console.log('Base URL:', baseUrl);
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then(cache => {
        console.log('Service Worker v25: Caching files');
      ...
      .then(() => {
        console.log('Service Worker v25: Skip waiting');
...
self.addEventListener('activate', event => {
  console.log('Service Worker v25: Activating...');
  event.waitUntil(
    caches.keys().then(cacheNames => {
      return Promise.all(
        cacheNames.map(cacheName => {
          if (cacheName !== CACHE_NAME) {
            console.log('Service Worker v25: Deleting old cache:', cacheName);
            return caches.delete(cacheName);
          }
        })
      );
    }).then(() => {
      console.log('Service Worker v25: Claiming clients');
