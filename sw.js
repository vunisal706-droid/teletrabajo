const CACHE_NAME = 'teletrabajo-capitulaciones-v27';
...
self.addEventListener('install', event => {
  console.log('Service Worker v27: Installing...');
  console.log('Base URL:', baseUrl);
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then(cache => {
        console.log('Service Worker v27: Caching files');
      ...
      .then(() => {
        console.log('Service Worker v27: Skip waiting');
...
self.addEventListener('activate', event => {
  console.log('Service Worker v27: Activating...');
  event.waitUntil(
    caches.keys().then(cacheNames => {
      return Promise.all(
        cacheNames.map(cacheName => {
          if (cacheName !== CACHE_NAME) {
            console.log('Service Worker v27: Deleting old cache:', cacheName);
            return caches.delete(cacheName);
          }
        })
      );
    }).then(() => {
      console.log('Service Worker v27: Claiming clients');
