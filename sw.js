const CACHE_NAME = 'teletrabajo-capitulaciones-v20';
...
self.addEventListener('install', event => {
  console.log('Service Worker v20: Installing...');
  console.log('Base URL:', baseUrl);
  console.log('URLs to cache:', urlsToCache);
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then(cache => {
        console.log('Service Worker v20: Caching files');
      ...
      .then(() => {
        console.log('Service Worker v20: Skip waiting');
...
self.addEventListener('activate', event => {
  console.log('Service Worker v20: Activating...');
  event.waitUntil(
    caches.keys().then(cacheNames => {
      return Promise.all(
        cacheNames.map(cacheName => {
          if (cacheName !== CACHE_NAME) {
            console.log('Service Worker v20: Deleting old cache:', cacheName);
            return caches.delete(cacheName);
          }
        })
      );
    }).then(() => {
      console.log('Service Worker v20: Claiming clients');
