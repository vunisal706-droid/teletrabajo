const CACHE_NAME = 'teletrabajo-capitulaciones-v6';
...
self.addEventListener('install', event => {
  ...
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then(cache => {
        console.log('Service Worker v6: Caching files');
      ...
      .then(() => {
        console.log('Service Worker v6: Skip waiting');
...
self.addEventListener('activate', event => {
  ...
  event.waitUntil(
    caches.keys().then(cacheNames => {
      ...
      console.log('Service Worker v6: Claiming clients');
